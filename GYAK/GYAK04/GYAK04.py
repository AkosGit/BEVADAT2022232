...
def dict_to_dataframe(inputDict) -> pd.core.frame.DataFrame:
...
# %%
def get_column(df: pd.DataFrame,string:str) -> pd.core.series.Series:
    new_df=df.copy()
...
def plot_area(df: pd.DataFrame)-> plt.figure:
    ...
    ax.pie(dfu["area"], labels=dfu["country"])
